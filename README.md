...
A lightweight desktop application for logging amateur radio contacts on Linux. Stores QSOs in SQLite and exports standard ADIF files for import into [N3FJP's Amateur Contact Log](https://www.n3fjp.com/aclog.html) or other logging software. Can also transfer QSOs directly to AC Log over your LAN.
...
- **flrig Integration** — Polls [flrig](http://www.w1hkj.com/) every 2 seconds for your radio's current frequency and mode. New QSOs are automatically populated from the radio. Connection status is shown in the status bar.
...
- **POTA Activation** — Activate POTA mode with a park reference, city, state/country, and grid square. These session-only settings override your home QTH defaults and are stamped on every QSO logged during the activation. The button stays green while active.
...
1. Click **Add QSO** (or Ctrl+N). If flrig is running, the frequency, band, and mode are pre-filled from your radio.
